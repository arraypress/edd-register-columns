 ...
 * Base Columns Class
 *
 * A lightweight class designed to simplify the registration of custom columns in Easy Digital Downloads.
 * This class focuses on display-only functionality, allowing you to easily add custom columns to EDD tables
 * without the complexity of inline editing or custom UI components.
 *
 ...
 * Class Columns
 *
 * Base class for registering custom columns in Easy Digital Downloads.
 *
 * @package ArrayPress\EDD\RegisterColumns
 */
abstract class Columns {
	 ...
	 *
	 * Examples: 'edd_order', 'edd_customer', 'edd_adjustment', 'download'
	 *
	 * @var string
	 */
	 ...
	 */
	public function __construct( array $columns, array $keys_to_remove = [] ) {
	...
	public function add_columns( array $columns ): void {
		...
		foreach ( $columns as $key => $column ) {
			...
			self::$columns[ $this->object_type ][ $key ] = wp_parse_args( $column, $default_column );
		}
	...
	/**
	 * Get columns array for the given object type.
	 *
	 * @param string $object_type Object type.
	 *
	 * @return array
	 */
	public static function get_columns( string $object_type ): array {
		return self::$columns[ $object_type ] ?? [];
	}
	 ...
	 */
	public function get_column_by_name( string $column_name, string $object_type ): ?array {
		$columns = self::get_columns( $object_type );
	...
	public function add_custom_column_styles(): void {
		...
		echo '<style>';
		foreach ( self::get_columns( $this->object_type ) as $key => $column ) {
	 ...
	 * @param mixed  $object      The object (order, customer, discount) or ID (download).
	 *
	 * @return string The rendered column content.
	 */
	abstract public function render_column_content( string $value, string $column_name, $object ): string;
	 ...
	 *
	 * Registers WordPress/EDD hooks for adding, sorting, and displaying custom columns.
	 *
	 * @return void
	 */
